...
# ---------- Evaluation Tool ----------
async def evaluation_tool(task_description: str, dataset_summary: str) -> str:
    """ML evaluation assistant tool"""
    ...
    try:
        result = await llm.chat(messages, temperature=0.0, max_tokens=800)
        await llm.close()
        return result
...
async def create_agent(coral_tools, agent_tools):
    ...
    prompt = ChatPromptTemplate.from_messages([
        (
            "system",
            f"""You are an Evaluation Agent interacting with the tools from Coral Server and having your own tools. Your task is to perform ML model evaluation instructions coming from any agent. 
            Follow these steps in order:
            1. Call wait_for_mentions from coral tools (timeoutMs: 30000) to receive mentions from other agents.
            2. When you receive a mention, keep the thread ID and the sender ID.
            3. Take 2 seconds to think about the content (instruction) of the message and check if it's related to ML model evaluation.
            4. If the instruction is for evaluation, use the evaluation_tool with the provided task description and dataset summary.
    ...
    model = init_chat_model(
        ...
        temperature=os.getenv("MODEL_TEMPERATURE", "0.0"),
        max_tokens=os.getenv("MODEL_MAX_TOKENS", "800"),
...
async def main():
    ...
    coral_params = {
        "agentId": agentID,
        "agentDescription": "ML Evaluation Agent that provides evaluation metrics and guidance for machine learning models"
    }
    ...
    print("Evaluation Agent Connection Established")
    ...
    agent_tools = [
        Tool(
            ...
            func=None,
            coroutine=evaluation_tool,
            description="Provide ML evaluation metrics and guidance. Args: task_description (str), dataset_summary (str)"
        )
    ]
