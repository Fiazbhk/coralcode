...
# ---------- Training Tool ----------
async def training_tool(selected_model: str, dataset_summary: str) -> str:
    """ML training assistant tool"""
    ...
    try:
        result = await llm.chat(messages, temperature=0.2, max_tokens=1024)
        await llm.close()
        return result
...
async def create_agent(coral_tools, agent_tools):
    ...
    prompt = ChatPromptTemplate.from_messages([
        (
            "system",
            f"""You are a Training Agent interacting with the tools from Coral Server and having your own tools. Your task is to perform ML model training instructions coming from any agent. 
            Follow these steps in order:
            1. Call wait_for_mentions from coral tools (timeoutMs: 30000) to receive mentions from other agents.
            2. When you receive a mention, keep the thread ID and the sender ID.
            3. Take 2 seconds to think about the content (instruction) of the message and check if it's related to ML model training.
            4. If the instruction is for training, use the training_tool with the provided selected model and dataset summary.
...
async def main():
    ...
    coral_params = {
        "agentId": agentID,
        "agentDescription": "ML Training Agent that generates training scripts and code for selected machine learning models"
    }
    ...
    print("Training Agent Connection Established")
    ...
    agent_tools = [
        Tool(
            name="training_tool",
            func=None,
            coroutine=training_tool,
            description="Generate ML training scripts and code. Args: selected_model (str), dataset_summary (str)"
        )
    ]
